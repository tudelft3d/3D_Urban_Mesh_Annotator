...
*/
    ...
    void CGAL::QGLViewer::beginSelection(const QPoint &point) {
        ...
        glScissor(point.x() * devicePixelRatio(), (camera()->screenHeight() - 1 - point.y()) * devicePixelRatio(), 1,
                  1);
    }
...
//  glPolygonMode(GL_FRONT_AND_BACK,GL_FILL);
        ...
        if (visualHint_ & 1) {
            ...
            auto p = camera()->projectedCoordinatesOf(camera()->pivotPoint());
            glViewport(GLint(p.x * devicePixelRatio() - size / 2),
                       GLint((height() - p.y) * devicePixelRatio() - size / 2), size, size);
            glScissor(GLint(p.x * devicePixelRatio() - size / 2),
                      GLint((height() - p.y) * devicePixelRatio() - size / 2), size, size);
...
X arrow is red, Y arrow is green and Z arrow is blue.*/
    ...
    void CGAL::QGLViewer::drawAxis(qreal length) {
        std::vector<float> data;
        data.resize(0);
        drawArrow(0.06, 0.12, 10, CGAL::qglviewer::Vec(0, 0, 0), CGAL::qglviewer::Vec(length, 0, 0),
                  CGAL::qglviewer::Vec(1, 0, 0), data);
        drawArrow(0.06, 0.12, 10, CGAL::qglviewer::Vec(0, 0, 0), CGAL::qglviewer::Vec(0, length, 0),
                  CGAL::qglviewer::Vec(0, 1, 0), data);
        drawArrow(0.06, 0.12, 10, CGAL::qglviewer::Vec(0, 0, 0), CGAL::qglviewer::Vec(0, 0, length),
                  CGAL::qglviewer::Vec(0, 0, 1), data);
        rendering_program_light.bind();
        vaos[AXIS].bind();
        vbos[Axis].bind();
...
\p size (OpenGL units) and \p nbSubdivisions define its geometry.*/
    ...
    void CGAL::QGLViewer::drawGrid(qreal size, int nbSubdivisions) {
        ...
        drawArrow(0.005 * size, 0.02 * size, 10, CGAL::qglviewer::Vec(0, 0, 0), CGAL::qglviewer::Vec(size, 0, 0),
                  CGAL::qglviewer::Vec(1, 0, 0), d_axis);
        drawArrow(0.005 * size, 0.02 * size, 10, CGAL::qglviewer::Vec(0, 0, 0), CGAL::qglviewer::Vec(0, size, 0),
                  CGAL::qglviewer::Vec(0, 1, 0), d_axis);
...
deleted using glDeleteTextures() since then. */
    ...
    CGAL_INLINE_FUNCTION
    QImage *CGAL::QGLViewer::takeSnapshot(CGAL::qglviewer::SnapShotBackground background_color, QSize finalSize,
                                          double oversampling, bool expand) {
        makeCurrent();
