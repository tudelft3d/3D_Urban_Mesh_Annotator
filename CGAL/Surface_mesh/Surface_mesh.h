...
public:
  ...
  bool join(const Surface_mesh& other)
  {
   ...
   /// returns the number of used and removed vertices in the mesh.
	...
	//********************Weixiao Update************************//
	void collect_garbage(std::map<Face_index, Face_index>&);
	//**********************************************************//
    /// @cond CGAL_DOCUMENT_INTERNALS
...
template <typename P>
void
Surface_mesh<P>::collect_garbage(std::map<Face_index, Face_index> &removed_unremoved)
{
	if (!has_garbage())
	{
		return;
	...
	if (nF > 0)
	{
		...
		while (1)
		{
			...
			removed_unremoved[Face_index(i0)] = Face_index(i1);
			// swap
			fprops_.swap(i0, i1);
		};
