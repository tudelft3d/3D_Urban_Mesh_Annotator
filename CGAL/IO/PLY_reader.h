...
namespace CGAL {
	...
	namespace internal
	{
		...
		{
			bool has_colors = false;
			std::string rtag = "", gtag = "", btag = "";

			if (((element.has_property<boost::uint8_t>("red") || element.has_property<float>("r")) &&
				(element.has_property<boost::uint8_t>("green") || element.has_property<float>("g")) &&
				(element.has_property<boost::uint8_t>("blue") || element.has_property<float>("b"))))
			{
				has_colors = true;
				/**/
				if (element.has_property<boost::uint8_t>("red"))
				...
				else if (element.has_property<float>("r"))
				{
			...
			for (std::size_t j = 0; j < element.number_of_items(); ++j)
			{
				for (std::size_t k = 0; k < element.number_of_properties(); ++k)
				{
					...
					{
						fi_segment_id.push_back(property->read<int>(in));
					}
					else if (has_colors && element.has_property<boost::uint8_t>("red") && element.property(k)->name() == rtag.c_str())
					{
						color.push_back(property->read<boost::uint8_t>(in));
					}
					else if (has_colors && element.has_property<boost::uint8_t>("green") && element.property(k)->name() == gtag.c_str())
					{
						color.push_back(property->read<boost::uint8_t>(in));
					}
					else if (has_colors && element.has_property<boost::uint8_t>("blue") && element.property(k)->name() == btag.c_str())
					{
						color.push_back(property->read<boost::uint8_t>(in));
					}
					else if (has_colors && element.has_property<float>("r") && element.property(k)->name() == rtag.c_str())
					{
						color.push_back(property->read<float>(in));
					}
					else if (has_colors && element.has_property<float>("g") && element.property(k)->name() == gtag.c_str())
					{
						color.push_back(property->read<float>(in));
					}
					else if (has_colors && element.has_property<float>("b") && element.property(k)->name() == btag.c_str())
					{
						color.push_back(property->read<float>(in));
					}
					else
				...
				if (color.size() == 3)
				{
					//std::cout << "color[0] = " << color[0] << "; color[1] = " << color[1] << "; color[2] = " << color[2] << std::endl;

					if (element.has_property<float>("r"))
						fcolors.push_back(Color_rgb(color[0] * 255.0f, color[1] * 255.0f, color[2] * 255.0f));
					else
						fcolors.push_back(Color_rgb(color[0], color[1], color[2]));
					color.clear();
				}
					...
					else if (element.has_property<float>("r"))
					{
						PLY::process_properties(element, new_face,
							std::make_pair(CGAL::make_nth_of_tuple_property_map<0>(new_face),
								PLY_property<std::vector<Integer> >(vertex_indices_tag)),
							std::make_pair(CGAL::make_nth_of_tuple_property_map<1>(new_face),
								PLY_property<float>(rtag.c_str())),
							std::make_pair(CGAL::make_nth_of_tuple_property_map<2>(new_face),
								PLY_property<float>(gtag.c_str())),
							std::make_pair(CGAL::make_nth_of_tuple_property_map<3>(new_face),
								PLY_property<float>(btag.c_str())));
						//fcolors.push_back(Color_rgb(get<1>(new_face) * 255.0f, get<2>(new_face) * 255.0f, get<3>(new_face) * 255.0f));
					}
				}
				else
					PLY::process_properties(element, new_face,
