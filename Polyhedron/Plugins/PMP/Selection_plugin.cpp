...
public Q_SLOTS:
	...
	void on_Selection_type_combo_box_changed(int index) {
		...
		for (Selection_item_map::iterator it = selection_item_map.begin(); it != selection_item_map.end(); ++it) {
			...
			if (index == 1)
			{
				...
				//ui_widget.operationsBox->setEnabled(true);
				//ui_widget.validateButton->setEnabled(true);
				/**********************Ziqian***************************/
			...
			/**********************************************************************/
			////***********************Weixiao update selection face*******************************//
			//if (index == 0)
			//{
			//	it->second->set_active_handle_type(static_cast<Active_handle::Type>(1));
			//	Q_EMIT save_handleType();
			//	ui_widget.lassoCheckBox->show();
			//	ui_widget.Select_all_NTButton->show();
			//	ui_widget.Add_to_selection_button->hide();
	...
	//	}
	//	/************************ziqian**********************/
	//	// Use the selected faces to build a segment
	//	case 10: {
	//		Scene_polyhedron_selection_item* selection_item = getSelectedItem<Scene_polyhedron_selection_item>();
	//		if (!selection_item) {
	//			print_message("Error: there is no selected polyhedron selection item!");
	...
	//			return;
	//		}
	//		if (!selection_item->put_selected_faces_into_one_segment()) {
	//			print_message("Error: selection involves more than one segment.");
	//		}
	//		selection_item->invalidateOpenGLBuffers();
	//		selection_item->itemChanged();
	//		break;
	...
	//			ui_widget.Selection_type_combo_box->currentIndex() == 5),
	...
	void filter_operations()
	{
		...
		//QString current_op = ui_widget.operationsBox->currentText();
		//ui_widget.operationsBox->clear();
		//bool facts = (ui_widget.Selection_type_combo_box->currentIndex() == 1) && !selection_item->selected_facets.empty();
		//if (facts)
		//{
		//	ui_widget.operationsBox->addItem(operations_strings[10]);
		//}
