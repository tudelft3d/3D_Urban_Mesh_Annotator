...
public:
    ...
    void compute_bbox() const {
        ...
        for (Selection_set_edge::const_iterator e_it = selected_edges.begin();
            ...
            e_bbox = e_bbox +
                     get(vpm, target(opposite(halfedge(*e_it, *polyhedron()), *polyhedron()), *polyhedron())).bbox();
        ...
        for (Selection_set_facet::const_iterator f_it = selected_facets.begin();
            ...
            BOOST_FOREACH(fg_halfedge_descriptor he,
                          halfedges_around_face(halfedge(fd, *polyhedron()), *polyhedron())) {
    ...
    template<class HandleType>
    // use fg_vertex_descriptor, fg_face_descriptor, fg_edge_descriptor
    boost::optional<std::size_t> get_minimum_isolated_component() {
        Selection_traits<HandleType, Scene_polyhedron_selection_item> tr(this);
    ...
    void reduce_selection(unsigned int steps) {
        ...
        BOOST_FOREACH(Handle h, tr.container()) mark[tr.id(h)] = true;
        ...
        for (Selection_set_edge::iterator eit = selected_edges.begin(); eit != selected_edges.end();) {
            if (//both incident faces will be erased
                    (selected_facets.find(face(halfedge(*eit, *polyhedron()), *polyhedron())) != selected_facets.end()
                     && selected_facets.find(
                            face(opposite(halfedge(*eit, *polyhedron()), *polyhedron()), *polyhedron())) !=
                        selected_facets.end())
                    //OR eit is a boundary edge and its incident face will be erased
                    || (is_border(halfedge(*eit, *polyhedron()), *polyhedron())
                        && (selected_facets.find(face(halfedge(*eit, *polyhedron()), *polyhedron())) !=
                            selected_facets.end()
                            || selected_facets.find(
                            face(opposite(halfedge(*eit, *polyhedron()), *polyhedron()), *polyhedron())) !=
                               selected_facets.end()))) {
    ...
    bool export_selected_facets_as_polyhedron(Face_graph *out) {
        ...
        for (Selection_set_facet::iterator fb = selected_facets.begin(); fb != selected_facets.end(); ++fb) {
            BOOST_FOREACH(fg_halfedge_descriptor hb,
                          halfedges_around_face(halfedge(*fb, *polyhedron()), *polyhedron())) {
    ...
    void select_sharp_edges(const double angle) {
        ...
        boost::property_map<Face_graph, CGAL::edge_is_feature_t>::type is_feature = get(CGAL::edge_is_feature,
                                                                                        *polyhedron());
...
protected:
    ...
    void has_been_selected(const std::set<HandleType> &selection) {
        ...
        if (get_active_handle_type() == Active_handle::CONNECTED_COMPONENT) {
            ...
            std::vector<bool> mark(tr.size(), false);
            BOOST_FOREACH(fg_edge_descriptor e, selected_edges) mark[tr.id(e)] = true;
            ...
            CGAL::Polygon_mesh_processing::connected_component(
                    ...
                    *polyhedron(),
                    ...
                    CGAL::Polygon_mesh_processing::parameters::edge_is_constrained_map(
                            Is_selected_property_map<fg_edge_descriptor, PM>(mark,
                                                                             get(boost::edge_index, *polyhedron()))));
...
public:
    ...
    selected_edges_pmap(std::vector<bool> &mark) {
        ...
        return Is_selected_property_map<fg_edge_descriptor, Face_graph_edge_index_pm>(mark, get(boost::edge_index,
                                                                                                *polyhedron()));
    }
